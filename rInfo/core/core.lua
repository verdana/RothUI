...
f1.text = rsiCreateFontString(f1, cfg.fontSize)
f2.text = rsiCreateFontString(f2, cfg.fontSize)
f3.text = rsiCreateFontString(f3, cfg.fontSize)
...
local function rsiClearGarbage()
    ...
    UpdateAddOnMemoryUsage()
    local after = gcinfo()
    print("|cffffff00Cleaned |r" .. memformat(before - after))
end
...
local function rsiShowMemTooltip(self)
    ...
    GameTooltip:SetPoint("TOPRIGHT", UIParent, "TOPRIGHT", 0, 0)
    ...
    local nr = 0
    UpdateAddOnMemoryUsage()
    GameTooltip:AddLine("插件内存占用排行", color.r, color.g, color.b)
    GameTooltip:AddLine(" ")
    ...
    for _, entry in pairs(addons) do
        if nr < cfg.maxAddons then
            GameTooltip:AddDoubleLine(entry.name, memformat(entry.memory), 1, 1, 1, ColorGradient(entry.memory / 1024, 0, 1, 0, 1, 1, 0, 1, 0, 0))
            nr = nr + 1
        end
    end
    GameTooltip:AddLine(" ")
    GameTooltip:AddDoubleLine("总计", memformat(total), color.r, color.g, color.b, color.r, color.g, color.b)
    GameTooltip:AddDoubleLine("总计 包括暴雪插件", memformat(blizz), color.r, color.g, color.b, color.r, color.g, color.b)

    -- Legion Currencies
    GameTooltip:AddLine(" ")

    for i = 1, #cfg.showCurrencies do
        local name, amount = GetCurrencyInfo(cfg.showCurrencies[i])
        GameTooltip:AddDoubleLine(name, amount, 0.8, 0.6, 0, color.r, color.g, color.b)
    end
